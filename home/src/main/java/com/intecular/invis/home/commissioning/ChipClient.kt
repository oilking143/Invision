...
class ChipClient @Inject constructor(@ApplicationContext context: Context) {
    ...
    private val VENDOR_ID = 0x146A

    val DEFAULT_TIMEOUT = 1000

    var deviceId = generateNextDeviceId()
            ...
            val callback: ReportCallback =
                object : ReportCallback {
                    ...
                    override fun onReport(nodeState: NodeState?) {
                        ...
                        Timber.d("ReadAttributes Success!!")
                        nodeState?.let {
                            for (path in attributePaths) {

                                var endpoint: Int = path.endpointId.id.toInt()
                                Timber.d("ReadAttributes Success EndPoint:${path.endpointId.id.toInt()}, ClusterId:${path.clusterId.id}, AttributeId:${path.attributeId.id}")
                                states[path] =
                                    it.getEndpointState(endpoint)!!
...
//                sensor += Sensor(deviceId, s, c, r)
    ...
    private fun stripLinkLocalInIpAddress(ipAddress: String): String {
